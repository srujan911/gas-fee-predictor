...
# Create a fallback heatmap that resembles Etherscan's
def create_fallback_heatmap(output_path):
    # Days of the week
    ...
    # Realistic Etherscan data with values that can go below 1 GWEI
    # These values are in GWEI and represent typical gas fees by day and hour
    data = np.array([
        # Monday - values based on Etherscan patterns with realistic lows
    ...
    plt.title('Ethereum Gas Fee Heatmap by Day and Hour (UTC)', fontsize=16, pad=20)
    plt.xlabel('Hour of Day (UTC)', fontsize=12, labelpad=10)
    ...
    plt.figtext(0.5, 0.01, "Source: Etherscan Gas Fee Heatmap (UTC Timezone)", 
...
if __name__ == "__main__":
    ...
    output_path = os.path.join(output_dir, 'etherscan_heatmap_fallback.png')
    create_fallback_heatmap(output_path)
